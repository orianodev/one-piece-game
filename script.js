...
// PLAYER
class Player {
    constructor(id, hp, x, y, direction, speed, attackSpeed, score, spriteUrl, thrownProjectile, opponentPosition) {
        this.id = id;
        this.hp = hp;
        this.x = x;
        this.y = y;
...
pickA.addEventListener("click", (e) => {
    thisPlayer = new Player("A", 100, defaultPosition.A.x, defaultPosition.A.y, "right", 10, 10, 0, "images/luffy.png", [], defaultPosition.B);
    opponentPlayer = new Player("B", 100, defaultPosition.B.x, defaultPosition.B.y, "left", 10, 10, 0, "images/zoro.png", [], defaultPosition.A);
    showPlayer.innerText = thisPlayer.id;
});
pickB.addEventListener("click", (e) => {
    thisPlayer = new Player("B", 100, defaultPosition.B.x, defaultPosition.B.y, "left", 10, 10, 0, "images/zoro.png", [], defaultPosition.A);
    opponentPlayer = new Player("A", 100, defaultPosition.A.x, defaultPosition.A.y, "right", 10, 10, 0, "images/luffy.png", [], defaultPosition.B);
    showPlayer.innerText = thisPlayer.id;
});
...
function reDrawAll() {
    ...
    thisPlayer.draw();
    opponentPlayer.draw();
    thisPlayer.thrownProjectiles.forEach((projectile) => projectile.draw());
    opponentPlayer.thrownProjectiles.forEach((projectile) => projectile.draw());
}
socket.on("move", () => {
    console.log("Received move instruction.");
    thisPlayer.thrownProjectiles.forEach((projectile) => projectile.move());
    opponentPlayer.thrownProjectiles.forEach((projectile) => projectile.move());
    reDrawAll();
});
socket.on("update", (msg) => {
    if (thisPlayer.id == "A") {
        thisPlayer = new Player("A", msg.A.hp, msg.A.x, msg.A.y, msg.A.direction, msg.A.speed, msg.A.attackSpeed, msg.A.score, msg.A.spriteUrl, rebuildProjectileArray(msg.A.thrownProjectiles), { x: msg.B.x, y: msg.B.y });
        opponentPlayer = new Player("B", msg.B.hp, msg.B.x, msg.B.y, msg.B.direction, msg.B.speed, msg.B.attackSpeed, msg.B.score, msg.B.spriteUrl, rebuildProjectileArray(msg.B.thrownProjectiles), { x: msg.A.x, y: msg.A.y });
    }
    else if (thisPlayer.id = "B") {
        thisPlayer = new Player("B", msg.B.hp, msg.B.x, msg.B.y, msg.B.direction, msg.B.speed, msg.B.attackSpeed, msg.B.score, msg.B.spriteUrl, rebuildProjectileArray(msg.B.thrownProjectiles), { x: msg.A.x, y: msg.A.y });
        opponentPlayer = new Player("A", msg.A.hp, msg.A.x, msg.A.y, msg.A.direction, msg.A.speed, msg.A.attackSpeed, msg.A.score, msg.A.spriteUrl, rebuildProjectileArray(msg.A.thrownProjectiles), { x: msg.B.x, y: msg.B.y });
    }
